...
@interface SGDropMenu()<UITableViewDelegate,UITableViewDataSource>
/** 蒙版 */
...
- (instancetype)initWithFrame:(CGRect)frame ArrowOffset:(CGFloat)arrowOffset TitleArr:(NSArray *)titleArr ImageArr:(NSArray *)imageArr Type:(SGDropMenuType)type LayoutType:(SGDropMenuLayoutType)layoutType RowHeight:(CGFloat)rowHeight Delegate:(id<SGDropMenuDelegate>)delegate {
    if (self = [super initWithFrame:frame]) {
        ...
        for (int i = 0; i < titleArr.count; i++) {
            ...
            SGDropMenuModel *model = [SGDropMenuModel dropMenuWithDictonary:tempDict];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    SGDropMenuCell *cell = [SGDropMenuCell dropMenuCellWithTableView:tableView];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    SGDropMenuModel *model = self.titleImageArrM[indexPath.row];
...
#pragma mark - 类型
- (void)setLayoutType:(SGDropMenuLayoutType)LayoutType {
    _LayoutType = LayoutType;
    for (SGDropMenuModel *model in self.titleImageArrM) {
...
@interface SGDropMenuCell()
/** 屏幕中心点 */
@property (nonatomic, assign) CGFloat screenCenter;
@end

@implementation SGDropMenuCell

+ (instancetype)dropMenuCellWithTableView:(UITableView *)tableView {
    SGDropMenuCell *cell = [tableView dequeueReusableCellWithIdentifier:kDropMenuCellID];
    if (!cell) {
        cell = [[SGDropMenuCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:kDropMenuCellID];
    }
    return cell;
}
...
- (void)layoutSubviews {
    ...
    if (self.model.layoutType == SGDropMenuLayoutTypeTitle) {
...
- (void)setModel:(SGDropMenuModel *)model {
    _model = model;
    if (model.layoutType == SGDropMenuLayoutTypeTitle) {
        self.titleL.frame = CGRectMake(10, self.screenCenter - 10, self.frame.size.width, 20);
    } else {
        self.imageIV.image = [UIImage imageNamed:model.image];
    }
    if (model.type == SGDropMenuTypeQQ) {
        self.titleL.textColor = [UIColor blackColor];
        self.line1.backgroundColor = [UIColor lightGrayColor];
    } else {
...
@interface SGDropMenuModel()
@end
@implementation SGDropMenuModel
