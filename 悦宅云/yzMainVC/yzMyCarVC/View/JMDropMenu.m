...
- (instancetype)initWithFrame:(CGRect)frame ArrowOffset:(CGFloat)arrowOffset TitleArr:(NSArray *)titleArr RowHeight:(CGFloat)rowHeight Delegate:(id<JMDropMenuDelegate>)delegate {
    if (self = [super initWithFrame:frame]) {
        ...
        for (int i = 0; i < titleArr.count; i++) {
            ...
            JMDropMenuModel *model = [JMDropMenuModel dropMenuWithDictonary:titleArr[i]];
...
+ (instancetype)showDropMenuFrame:(CGRect)frame ArrowOffset:(CGFloat)arrowOffset TitleArr:(NSArray *)titleArr RowHeight:(CGFloat)rowHeight Delegate:(id<JMDropMenuDelegate>)delegate{
    return [[self alloc] initWithFrame:frame ArrowOffset:arrowOffset TitleArr:titleArr RowHeight:rowHeight Delegate:delegate];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([_delegate respondsToSelector:@selector(didSelectRowAtIndex:Title:)]) {
        [_delegate didSelectRowAtIndex:model.idStr Title:model.name];
    }
    
    if (self.blockSelectedMenu) {
        self.blockSelectedMenu(model);
    }
    [self removeDropMenu];
}
