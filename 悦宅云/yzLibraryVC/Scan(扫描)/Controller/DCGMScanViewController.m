...
- (void)setUpTopView
{
    _cameraTopView = [[DCCameraTopView alloc] initWithFrame:CGRectMake(0, 0, mDeviceWidth, DCTopNavHeight)];
    WEAKSELF
...
- (void)setUpBottomView
{
    ...
    bottomView.frame = CGRectMake(0, mDeviceWidth - 65, mDeviceWidth, 50);
    ...
    supLabel.frame = CGRectMake(0, 0, mDeviceWidth, 20);
    [bottomView addSubview:supLabel];
    
    NSArray *titles = @[@"条形码",@"二维码"];//@"快递单",@"物价码",
    NSArray *images = @[@"",@""];//@"",@""
    CGFloat btnW = (mDeviceWidth - 80) / titles.count;
